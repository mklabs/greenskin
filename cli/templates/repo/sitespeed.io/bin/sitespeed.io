...
do
  ...
  do
    ...
    ($BROWSERTIME -f json --compact --raw -b ${BROWSERS_ARRAY[i]} $PROXY_BROWSER_TIME -n $NUMBER_OF_RUNS -o "$REPORT_DATA_METRICS_DIR/${BROWSERS_ARRAY[i]}/$pagefilename.json" -ua "\"$USER_AGENT\"" -w $VIEWPORT "$url" 3>&1 1>&2 2>&3 | grep -v '^Starting' ) 3>&1 1>&2 2>&3 >> $REPORT_DATA_DIR/error.log
