...
var mochaRunner = path.join(__dirname, '../server/test/mocha-test.js');
mochaSteps[0].body = fs.readFileSync(path.join(__dirname, '../server/test/mocha-stepfile.js'), 'utf8');

var app = express();

var featuredir = path.join(__dirname, '../server/test/features');
var stepdir = path.join(__dirname, '../server/test/steps');
var tmpdir = path.join(__dirname, '../server/tmp');
...
app.get(/^\/run-feature\/(.+)\/?$/, function(req, res, next) {
  ...
  phantom.stdout.on('data', function(data) {
    data = data + '';
    if (app.ws) app.ws.sockets.emit('log', { line: data });
  });
