...
async.series(
	...
	function(err, results) {
		...
		if (results[0] !== false) {
			...
			reporter = require('phantomas/core/reporter')(results, options);
			options.reporter = 'tap';
			tap = require('phantomas/core/reporter')(results, options);

			debug('Calling a reporter...');

			var resTap = tap.render(doneFn);
			if (typeof resTap !== 'undefined') {
				process.stdout.write(resTap);
			}
