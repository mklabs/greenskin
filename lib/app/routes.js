...
var FilmstripComponent = require('./components/filmstrip');
...
class Routes extends Router {
  ...
  create(req, res, next) {
    ...
    if (!name) return this.render(req, res, 'create', {
      job: job
    });

    // Fetch the job, fill the form with it
    this.api.query({ name: name })
      .catch(next)
      .then((jobs) => {
        if (!jobs.length) return res.redirect('/');
        job = jobs[0];
        this.render(req, res, 'create', { job: Object.assign({}, job.data, {
          name: job.name,
          config: JSON.stringify({
            asserts: job.data.asserts || this.defaultAsserts
          }, null, 2)
        }) });
      });
  }
    ...
    this.api
      ...
      .then((doc) => {
        ...
        return this.api.create(item).then(() => {
          return res.redirect('/job/' + encodeURIComponent(body.name));
        })
      })
      .catch(next);
  }
    ...
    this.api.query({ name: name })
      ...
      .then((doc) => {
        ...
        if (doc.lastRunResults) {
          ...
          if (!url && doc.urls.length) doc.urls[0].selected = true;
        }


        var filmstrip = new FilmstripComponent(doc);
        filmstrip.build()
          .catch(next)
          .then(() => {
            doc.filmstripWidth = doc.filmstripWidth + 'px';
