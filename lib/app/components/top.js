...
module.exports = class name {
  ...
  build() {
    ...
    top.push({
      label: '<strong>Next run</strong>',
      value: moment(Date.parse(doc.nextRunAt)).fromNow(),
      icon: 'ios-timer-outline'
    });

    top.push({
      label: '<strong>Last run (started/completed)</strong>',
      value: moment(Date.parse(doc.lastRunAt)).fromNow() + ' / ' +
